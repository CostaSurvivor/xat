...
export async function GET() {
  ...
  const data = { exportedAt: new Date(), pushDevices, albums, stories, groups, testimonialsReceived, testimonialsWritten, events, eventRsvps, visitsReceived, visitsMade, liveStreams, liveMessages: liveMessages.map((m) => ({ ...m, amount: m.amount ?? undefined })), profile, persons, consents, posts, comments, roomMessages: messages, privateMessagesSent: pms, follows, blocks, payments, inventory, media, accessLogs: access };
