...
import { exchangeCode, googleEnabled, publicOrigin, putPending, takeFlow } from "@/server/google";
import { audit } from "@/server/notify";

const go = async (path: string) => NextResponse.redirect(`${await publicOrigin()}${path}`);

export async function GET(req: Request) {
  if (!googleEnabled()) return await go("/login?erro=google-off");
  const url = new URL(req.url);
  const flow = await takeFlow();
  const state = url.searchParams.get("state");
  const code = url.searchParams.get("code");
  if (!flow || !state || state !== flow.state || !code) return await go("/login?erro=google-falhou");
  const { ip, deviceId } = await clientInfo();
  if (!limiter("google-cb", 20, 20 / 900).take(ip)) return await go("/login?erro=muitas-tentativas");
  ...
  if (typeof claims === "string") return await go("/login?erro=google-falhou");

  // vincular Google a uma conta já logada
  if (flow.mode === "link") {
    const me = await getCurrentUser();
    if (!me) return await go("/login");
    const owner = await db.user.findFirst({ where: { googleSub: claims.sub }, select: { id: true } });
    if (owner && owner.id !== me.id) return await go("/conta?google=em-uso");
    await db.user.update({ where: { id: me.id }, data: { googleSub: claims.sub } });
    await audit(me.id, "account.google.link", "User", me.id);
    return await go("/conta?google=vinculado");
  }

  const user = await db.user.findFirst({ where: { googleSub: claims.sub } });
  if (user) {
    if (user.status === "BANNED") return await go("/login?erro=banido");
    if (user.status === "DELETED") return await go("/login?erro=excluida");
    if (user.role === "USER" && (await isFingerprintBanned({ deviceId }))) return await go("/login?erro=bloqueado");
    if (user.twoFactorEnabled && user.twoFactorSecret) {
      await putPending("TWOFA", { userId: user.id, next: flow.next });
      return await go("/login/google");
    }
    await createSession(user.id);
    return await go(flow.next);
  }
