...
const TX_PT: Record<string, string> = {
  TIP: "Gorjeta (ao vivo)", TRADE: "Troca", PURCHASE_CREDIT: "Recarga Pix", VIP_BONUS: "Bônus de assinatura", ITEM_PURCHASE: "Compra na loja", GIFT_COINS: "Presente", GIFT_ITEM: "Item de presente", ADMIN_ADJUSTMENT: "Ajuste", REFUND: "Estorno",
};
    ...
    <div className="mx-auto max-w-3xl space-y-5">
      ...
      <section>
        ...
        <ActionForm action={applyCoupon} className="mb-3 flex flex-wrap items-center gap-2" okText={coupon ? `Cupom ${coupon.code} aplicado: +${coupon.bonusPercent}% de ${CURRENCY_NAME}!` : "Cupom removido."}>
          ...
          {coupon && <span className="text-xs text-green-700">🎟️ {coupon.code}: +{coupon.bonusPercent}% nas próximas recargas</span>}
        </ActionForm>
        {!isVerified(user) ? (
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {packages.map((p) => (
              <form key={p.id} action={createPixPayment.bind(null, p.id)} className="card flex flex-col items-center p-4 text-center">
                ...
                {p.bonusCoins > 0 && <p className="text-xs text-green-700">+{p.bonusCoins} bônus</p>}
          ...
          <ul className="divide-y divide-line text-sm">
            {payments.map((p) => (
              <li key={p.id} className="flex items-center gap-2 py-2">
                ...
                {p.status === "PENDING" ? <Link href={`/carteira/pix/${p.id}`} className="text-gold underline">{ST_PT[p.status]}</Link> : <span className={p.status === "PAID" ? "text-green-700" : "text-mute"}>{ST_PT[p.status]}</span>}
              </li>
            ))}
          </ul>
        </section>
          ...
          <ul className="divide-y divide-line text-sm">
            {entries.map((e) => (
              <li key={e.id} className="flex items-center gap-2 py-2">
                ...
                <span className={`w-20 text-right font-semibold ${e.amount > 0 ? "text-green-700" : "text-red-700"}`}>{e.amount > 0 ? "+" : ""}{e.amount}</span>
              </li>
            ))}
          </ul>
        )}
