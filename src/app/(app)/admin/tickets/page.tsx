    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {Object.entries(TICKET_STATUS).map(([k, v]) => (
          <Link key={k} href={`?status=${k}`} className={`rounded-full px-3 py-1 text-xs ${k === status ? "bg-wine" : "border border-line"}`}>{v.label}</Link>
        ))}
      </div>
      <div className="card divide-y divide-line">
        {tickets.length === 0 && <p className="p-4 text-sm text-mute">Nenhum chamado aqui.</p>}
        {tickets.map((t) => (
          <Link key={t.id} href={`/admin/tickets/${t.id}`} className="flex flex-wrap items-center gap-3 p-3 text-sm hover:bg-white/5">
