...
const schema = z.object({
  targetType: z.enum(["USER", "POST", "COMMENT", "ROOM_MESSAGE", "PRIVATE_MESSAGE", "MEDIA", "ROOM", "LIVE", "EVENT", "TESTIMONIAL", "CONTO", "CONTO_COMMENT", "PLACE_REVIEW"]),
...
async function snapshot(type: ReportTargetType, id: string) {
  switch (type) {
    ...
    case "PRIVATE_MESSAGE": {
      ...
      return { userId: m?.senderId, evidence: m && { body: m.body, mediaId: m.mediaId, ...(m.audioSecs != null ? { audio: true, audioSecs: m.audioSecs } : {}) }, mediaIds: m?.mediaId ? [m.mediaId] : [] };
    }
    case "MEDIA": {
    ...
    case "TESTIMONIAL": {
      ...
      return { userId: t?.authorId, evidence: t && { body: t.body, profileId: t.profileId, metInPerson: t.metInPerson }, mediaIds: [] };
    }
    case "CONTO": {
      const c = await db.conto.findUnique({ where: { id } });
      return { userId: c?.authorId, evidence: c && { title: c.title, category: c.category, body: c.body.slice(0, 5000) }, mediaIds: [] };
    }
    case "CONTO_COMMENT": {
      const c = await db.contoComment.findUnique({ where: { id } });
      return { userId: c?.authorId, evidence: c && { body: c.body, contoId: c.contoId }, mediaIds: [] };
    }
    case "PLACE_REVIEW": {
      const r = await db.placeReview.findUnique({ where: { id } });
      return { userId: r?.userId, evidence: r && { stars: r.stars, body: r.body, placeId: r.placeId }, mediaIds: [] };
    }
    case "LIVE": {
