      ...
      <div className="space-y-2">
        {t.messages.map((m) => (
          <div key={m.id} className={`flex ${m.fromStaff === viewerIsStaff ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[85%] rounded-2xl px-4 py-2 text-sm ${m.fromStaff ? "bg-pink-100" : "bg-panel2"}`}>
              <p className="mb-0.5 text-[11px] text-fg/60">{m.fromStaff ? "🛡️ Administração" : `@${t.user.nick}`} · {m.createdAt.toLocaleString("pt-BR")}</p>
