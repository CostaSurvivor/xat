...
export async function OnlineRooms() {
  ...
  return (
    <div className="card sticky top-20 p-4">
      {lives.length > 0 && (
        <div className="mb-4">
          <h3 className="mb-2 font-semibold text-red-400"><span className="live-dot">●</span> Ao vivo agora</h3>
          <ul className="space-y-1">
            {lives.map((l) => (
              <li key={l.id}>
                <Link href={`/ao-vivo/${l.id}`} className="flex items-center justify-between gap-2 rounded-lg px-2 py-1.5 text-sm hover:bg-white/5">
